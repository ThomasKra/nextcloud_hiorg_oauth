    ...
    switch ($this->apiVersion) {
      ...
      case 'v2':
        ...
        if ($gruppe = $data_selbst->get('data')) {
          $this->log->debug('Gruppen: ' . json_encode($gruppe->attributes->gruppen_namen));
          $this->log->debug('Gruppensumme: ' . array_sum(array_keys(($gruppe->attributes->gruppen_namen))));
          $userProfile->data['gruppe'] = array_sum(array_keys(($gruppe->attributes->gruppen_namen)));
        } else {
          $this->log->debug('Keine Informationen zur Gruppenzugehörigkeit gefunden');
        }
        break;
      default:
        throw ('Version not existent');
